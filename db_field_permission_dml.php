...
function db_field_permission_insert(&$error_message = '') {
	...
	if(strlen(Request::val('SelectedID'))) db_field_permission_copy_children($recID, Request::val('SelectedID'));
...
function db_field_permission_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $separateDV = 0, $TemplateDV = '', $TemplateDVP = '') {
	...
	$filterer_table_field = Request::val('filterer_table_field');
	...
	if(is_file(__DIR__ . '/hooks/db_field_permission.fieldstate.csv')) {
		$fieldstate_data = addslashes(implode('', @file(__DIR__ . '/hooks/db_field_permission.fieldstate.csv')));
		$combo_fieldstate->ListItem = array_trim(explode('||', entitiesToUTF8(convertLegacyOptions($fieldstate_data))));
		$combo_fieldstate->ListData = $combo_fieldstate->ListItem;
	} else {
		$combo_fieldstate->ListItem = array_trim(explode('||', entitiesToUTF8(convertLegacyOptions("locked;;hidden"))));
		$combo_fieldstate->ListData = $combo_fieldstate->ListItem;
	}
	...
	if($selected_id) {
		...
		$combo_fieldstate->SelectedText = (isset($filterField[1]) && $filterField[1] == '4' && $filterOperator[1] == '<=>' ? $filterValue[1] : '');
	}
	...
	$lookup_fields = ['groupID' => ['view_membership_groups', 'GroupID'], 'table_field' => ['tmp_tables_fields', 'Table field'], ];
	...
	$templateCode .= loadView('db_field_permission-ajax-cache', ['rdata' => $rdata, 'jdata' => $jdata]);
