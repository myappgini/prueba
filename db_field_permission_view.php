...
// Download AppGini for free from https://bigprof.com/appgini/download/
	...
	@include_once("{$currDir}/hooks/db_field_permission.php");
	include_once("{$currDir}/db_field_permission_dml.php");
	...
	if($perm['view'] == 1 || ($perm['view'] > 1 && $DisplayRecords == 'user' && !$_REQUEST['NoFilter_x'])) { // view owner only
		...
		$x->QueryWhere = "WHERE `db_field_permission`.`ID_field_permissions`=`membership_userrecords`.`pkValue` AND `membership_userrecords`.`tableName`='db_field_permission' AND LCASE(`membership_userrecords`.`memberID`)='" . getLoggedMemberID() . "'";
	} elseif($perm['view'] == 2 || ($perm['view'] > 2 && $DisplayRecords == 'group' && !$_REQUEST['NoFilter_x'])) { // view group only
		$x->QueryFrom .= ', `membership_userrecords`';
		$x->QueryWhere = "WHERE `db_field_permission`.`ID_field_permissions`=`membership_userrecords`.`pkValue` AND `membership_userrecords`.`tableName`='db_field_permission' AND `membership_userrecords`.`groupID`='" . getLoggedGroupID() . "'";
	} elseif($perm['view'] == 3) { // view all
		// no further action
	} elseif($perm['view'] == 0) { // view none
		$x->QueryFields = ['Not enough permissions' => 'NEP'];
		$x->QueryFrom = '`db_field_permission`';
		$x->QueryWhere = '';
		$x->DefaultSortField = '';
	}
