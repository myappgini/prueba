		...
		if($schema === null) {
			...
			$schema = [
				'contacto' => [
					...
					'user' => ['appgini' => "VARCHAR(40) NULL"],
					'rango' => ['appgini' => "VARCHAR(40) NULL"],
					'date' => ['appgini' => "DATE NULL"],
				],
				'salary' => [
					'id' => ['appgini' => "INT UNSIGNED NOT NULL PRIMARY KEY AUTO_INCREMENT"],
					'contacto' => ['appgini' => "INT UNSIGNED NULL"],
					'monto' => ['appgini' => "VARCHAR(40) NULL"],
					'mes' => ['appgini' => "VARCHAR(40) NULL"],
					'nombre' => ['appgini' => "INT UNSIGNED NULL"],
					'rango' => ['appgini' => "INT UNSIGNED NULL"],
					'date' => ['appgini' => "INT UNSIGNED NULL DEFAULT '1'"],
				],
				'products' => [
					'id' => ['appgini' => "INT UNSIGNED NOT NULL PRIMARY KEY AUTO_INCREMENT"],
					'name' => ['appgini' => "VARCHAR(40) NULL"],
				...
				'todos' => [
					'id' => ['appgini' => "INT UNSIGNED NOT NULL PRIMARY KEY AUTO_INCREMENT"],
					'tarea' => ['appgini' => "TEXT NULL"],
					'dateInit' => ['appgini' => "DATE NULL"],
					'dateEnd' => ['appgini' => "DATE NULL"],
				],
			];
		}
	...
	function lookupQuery($tn, $lookupField) {
		...
		*/
		$lookupQuery = [
			...
			'salary' => [
				'contacto' => 'SELECT `contacto`.`id`, IF(CHAR_LENGTH(`contacto`.`name`) || CHAR_LENGTH(`contacto`.`user`), CONCAT_WS(\'\', `contacto`.`name`, \' - \', `contacto`.`user`), \'\') FROM `contacto` ORDER BY 2',
				'nombre' => 'SELECT `contacto`.`id`, `contacto`.`name` FROM `contacto` ORDER BY 2',
				'rango' => 'SELECT `contacto`.`id`, `contacto`.`rango` FROM `contacto` ORDER BY 2',
				'date' => 'SELECT `contacto`.`id`, if(`contacto`.`date`,date_format(`contacto`.`date`,\'%d/%m/%Y\'),\'\') FROM `contacto` ORDER BY 2',
			],
			'products' => [
			],
			'db_field_permission' => [
